...
function createWoodTexture() {
    ...
    ctx.fillStyle = '#e3c9a6'; // Light ash wood
    ...
    for(let i=0; i<30; i++) {
        ...
        ctx.moveTo(x, 0);
        ctx.bezierCurveTo(x + 20, 100, x - 20, 150, x, 256);
        ctx.stroke();
    }
...
function createStadium() {
    const standMat = new THREE.MeshPhongMaterial({ color: 0x444444 });
    const seatMat = new THREE.MeshPhongMaterial({ color: 0x113399 });
    const createStand = (width, depth, x, z, rot) => {
        const group = new THREE.Group();
        group.add(new THREE.Mesh(new THREE.BoxGeometry(width, 10, depth), standMat));
        for(let i=1; i<=6; i++) {
            const tier = new THREE.Mesh(new THREE.BoxGeometry(width, 4, depth - (i * 6)), seatMat);
            tier.position.set(0, 5 + (i * 4), -(i * 3));
            group.add(tier);
        }
        group.position.set(x, 5, z); group.rotation.y = rot;
        scene.add(group);
    };
    createStand(220, 70, -95, 0, Math.PI / 2);
    createStand(220, 70, 95, 0, -Math.PI / 2);
    createStand(140, 70, 0, 130, 0);
    const hill = new THREE.Mesh(new THREE.BoxGeometry(110, 6, 45), standMat);
    hill.position.set(0, 3, -120); scene.add(hill);
}
createStadium();

const pitchWidth = 90; const pitchLength = 145;
const grassMat = new THREE.MeshPhongMaterial({ color: 0x2d5e1e, shininess: 2 });
const pitch = new THREE.Mesh(new THREE.PlaneGeometry(pitchWidth + 50, pitchLength + 50), grassMat);
...
const createLine = (w, h, x, z) => {
    ...
    mesh.rotation.x = -Math.PI / 2; mesh.position.set(x, 0.02, z); scene.add(mesh);
};
...
function createGoal(zPos) {
    ...
    const netMat = new THREE.MeshPhongMaterial({ color: 0xffffff, transparent: true, opacity: 0.25, side: THREE.DoubleSide });
...
const jerseyMat = new THREE.MeshPhongMaterial({ color: 0xffcc00 });
keeper.add(new THREE.Mesh(new THREE.BoxGeometry(0.6, 1.1, 0.35), jerseyMat));
const head = new THREE.Mesh(new THREE.SphereGeometry(0.22, 16, 16), new THREE.MeshPhongMaterial({ color: 0xccaa88 }));
head.position.y = 0.8; keeper.add(head);
...
function createHurley() {
    ...
    const woodMat = new THREE.MeshPhongMaterial({ map: woodTexture, shininess: 10 });
    const gripMat = new THREE.MeshPhongMaterial({ color: 0x333333 }); // Black grip tape

    // Handle
    const handle = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.045, 1.0, 16), woodMat);
    handle.position.y = 0.4; group.add(handle);

    // Grip Tape
    const grip = new THREE.Mesh(new THREE.CylinderGeometry(0.048, 0.048, 0.4, 16), gripMat);
    grip.position.y = 0.7; group.add(grip);

    // Boss (Curved Head)
    const shape = new THREE.Shape();
    shape.moveTo(0, 0); shape.lineTo(0.12, 0);
    shape.quadraticCurveTo(0.28, 0.06, 0.34, 0.22);
    shape.lineTo(0.34, 0.42);
    shape.quadraticCurveTo(0.18, 0.48, 0, 0.35);
    shape.lineTo(0, 0);
...
const keys = {};
document.addEventListener('mousedown', (e) => { if(!controls.isLocked) controls.lock(); else if(e.button === 0) { state.isCharging = true; state.power = 0; } });
document.addEventListener('mouseup', (e) => { if(state.isCharging && e.button === 0) { strikeBall(); state.isCharging = false; document.getElementById('power-bar').style.width = '0%'; } });
...
function toggleSolo() {
    if (state.isSoloing) {
        ...
        // Drop the ball with current player velocity
        const dir = new THREE.Vector3();
        camera.getWorldDirection(dir);
        ballPhys.vel.copy(dir.multiplyScalar(0.1));
    } else {
        const dist = camera.position.distanceTo(ball.position);
        if (dist < 4) {
            state.isSoloing = true;
            createParticles(ball.position.clone(), 10, 0xffffff, 0.04, 0.1);
        }
    }
}
...
function strikeBall() {
    ...
    if (state.isSoloing || dist < 3.5) {
        ...
        const initialRot = hurleyGroup.rotation.x; hurleyGroup.rotation.x -= 0.9;
        setTimeout(() => hurleyGroup.rotation.x = initialRot, 150);
        
        const boost = state.isSoloing ? 1.3 : 1.0;
        const finalStrength = (0.2 + (state.power / 100) * 1.3) * boost;
        const dir = new THREE.Vector3();
        camera.getWorldDirection(dir);
...
function update() {
    if (controls.isLocked) {
        ...
        // Drop ball if sprinting/turning too hard while soloing
        if (state.isSoloing && speed > 0.3 && Math.random() < 0.005) {
            state.isSoloing = false;
        }
    }
    ...
    if (state.isSoloing) {
        // Ball follows hurley with a bit of wobble
        state.soloWobble += 0.15;
        const wobbleX = Math.sin(state.soloWobble) * 0.03;
        const wobbleY = Math.cos(state.soloWobble * 0.8) * 0.02;
        
        // Position relative to camera/hurley
        const ballPos = new THREE.Vector3(0.4 + wobbleX, -0.3 + wobbleY, -0.7);
        ballPos.applyQuaternion(camera.quaternion);
        ball.position.copy(camera.position).add(ballPos);
    ...
    if (keeper.position.x < targetX) keeper.position.x += 0.07;
    if (keeper.position.x > targetX) keeper.position.x -= 0.07;
    ...
    const checkGoal = (goalZ) => {
        if (Math.abs(ball.position.z - goalZ) < 0.6 && Math.abs(ball.position.x) < 3.25) {
            if (ball.position.y > 0.15 && ball.position.y < 2.5) {
                ...
                createParticles(new THREE.Vector3(0, 1, goalZ), 50, 0xffd700, 0.06, 0.5);
            } else if (ball.position.y >= 2.5 && ball.position.y < 12) {
                state.score += 1; updateScore(); resetBall();
                createParticles(new THREE.Vector3(0, 4, goalZ), 30, 0xffffff, 0.05, 0.4);
            }
        }
    };
